...
func CamelToKebab(s string) string {
	var kebabBuffer bytes.Buffer
	lastCaseUpper := false

	for _, r := range s {
		if unicode.IsUpper(r) {
			if !lastCaseUpper {
				if kebabBuffer.Len() > 0 {
					kebabBuffer.WriteRune('-')
				}
				kebabBuffer.WriteRune(unicode.ToLower(r))
			} else {
				kebabBuffer.WriteRune(unicode.ToLower(r))
			}
			lastCaseUpper = true
	...
	return kebabBuffer.String()
}

func KebabToLittleCamel(s string) string {
