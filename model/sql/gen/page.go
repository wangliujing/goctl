...
func genPage(table Table) (string, string, error) {
	camel := table.Name.ToCamel()
	text, err := pathx.LoadTemplate(category, pageTemplateFile, template.Page)
	if err != nil {
		return "", "", err
	}

	output, err := util.With("page").
		Parse(text).
	...
	text, err = pathx.LoadTemplate(category, pageMethodTemplateFile, template.PageMethod)
	if err != nil {
		return "", "", err
	}

	pageMethod, err := util.With("pageMethod").
		Parse(text).
		Execute(map[string]any{
			"upperStartCamelObject":     camel,
			"lowerStartCamelPrimaryKey": util.EscapeGolangKeyword(stringx.From(table.PrimaryKey.Name.ToCamel()).Untitle()),
